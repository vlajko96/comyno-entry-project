...
# Database: New postgresql user will be created (user: "comyno", password: "comyno") and new database will be created (database: "comynoDB"). You can connect to database using command "psql -U comyno -d comynoDB". Also, new tables will be created:
...
# QPID broker: 2 queues and 2 topics will be added ("send-queue", "reply-queue", "send-topic", "reply-topic") to qpid broker so we can use them for testing (more can be added using qpid-config command). Also, these queues and topics will be added to "Exchanges" table in database. 
...
- ./commands.sh build - builds postgres and qpid client libraries (libqpidmessagingcomyno.so and libpgclientcomyno.so) and puts them in ./output/lib folder
                      - builds unit tests (test-qpid and test-pgclient) and puts them in ./output/bin folder
                      - builds interactive test (test-cm) and puts him in ./output/bin folder
